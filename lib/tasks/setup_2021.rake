...
task :setup_2021 => :environment do
  ActiveRecord::Base.transaction do
    NFL_2021_SCHEDULE = 'db/NFL_2021_schedule.csv'
    ...
    Team.find_by(city: "Washington").update!(nickname: nil, full_name: "Washington Football Team")

    ### Games
    CSV.foreach(NFL_2021_SCHEDULE, {:headers=>:first_row}) do |row|
...
task :setup_2021_preseason => :environment do
  ActiveRecord::Base.transaction do
    NFL_2021_PRESEASON_SCHEDULE = 'db/NFL_2021_preseason_schedule.csv'

    puts 'Preparing database for 2021 NFL preseason...'

    puts 'Creating Year 2021 preseason...'
    year = Year.find_or_create_by(name: '2021 preseason')
    ...
    ### Games
    CSV.foreach(NFL_2021_PRESEASON_SCHEDULE, {:headers=>:first_row}) do |row|
      puts "Home: #{row[6].strip}"
      home = Team.find_by(full_name: row[6].strip)
      puts "Away: #{row[3].strip}"
      away = Team.find_by(full_name: row[3].strip)
